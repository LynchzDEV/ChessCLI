...
public class Gamepiece {
    ...
    ArrayList<String> moves; // List of possible valid moves
    ...
    public void createMoveSet(Board currentBoard) {
        ...
        if (this.getPiece().equalsIgnoreCase("P")) {
            ...
            if (this.getPlayer().equalsIgnoreCase("w")) { // CASE: White
                ...
                if (currentBoard.GBoard[pieceI + 1][pieceJ] == null) { // If the space in front of the pawn is free, it
                                                                       // can move there.
                    tempMove = Controller.indToPos(pieceI + 1, pieceJ);
                    this.moves.add(tempMove);
                }

                if ((pieceJ + 1) < 8) { // If the space to the diagonal of the pawn holds an opponent's piece, it can
                                        // take it
                ...
                if (pieceI == 4) { // If the pawn is on the 5th rank, it has a chance of capturing en passant
                    // If last piece moved is a pawn to either file next to the pawn on rank 5, make
                    // en passant a valid move
                    if (Controller.fileToInd(currentBoard.lastPieceMoved.getPosition()) == (pieceJ + 1)) {
                        if (currentBoard.lastPieceMoved.getPiece().equalsIgnoreCase("p")
                                && currentBoard.lastPieceMoved.movecount == 1) {
                            tempMove = Controller.indToPos(pieceI + 1, pieceJ + 1);
                            this.moves.add(tempMove);
                        }
                    }
        ...
        if (this.getPiece().equalsIgnoreCase("R")) { // CASE: Rook
            ...
            if (this.getPlayer().equalsIgnoreCase("w")) { // CASE: White
                ...
                for (int i = pieceI + 1; i < 8; i++) { // Check spaces to the north of rook for empty spaces/black
                                                       // pieces
                    if (currentBoard.GBoard[i][pieceJ] == null) {
                        tempMove = Controller.indToPos(i, pieceJ);
                        this.moves.add(tempMove);
                        continue;
                    } // If a black piece occupies a space in front of rook, add that space as a valid
                      // move, but stop iterating (rook cannot go through opponent)
        ...
        if (this.getPiece().equalsIgnoreCase("N")) { // CASE: knight
            ...
            if (this.getPlayer().equalsIgnoreCase("w")) { // CASE:white
                ...
                if ((iOffset < 8) && (jOffset < 8) && (iOffset > -1) && (jOffset > -1)) { // If space is empty or
                                                                                          // occupied by opponent, it is
                                                                                          // a valid move
        ...
        if (this.getPiece().equalsIgnoreCase("B")) { // Case: Bishop
            ...
            if (this.getPlayer().equalsIgnoreCase("w")) {// Case: White
                ...
                for (int i = 1; i < 8; i++) { // Test diagonals starting from 1 space away to the Northeast. Max is 7.
                    ...
                    // If the space is on the board and empty or contains opponent piece, add to
                    // valid move list
